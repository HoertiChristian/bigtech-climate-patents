"""
Lens Patent API — Query Y02 climate patents for Big Tech firms (2010–2024).
...
Key distinction:
  ...
  - CPC prefix:     query_string with "class_cpc.symbol:Y02*" (wildcard match)
"""
...
# Fuzzy match threshold (0–100). 80 = moderate — catches typos and slight
# variations while avoiding false positives across different companies.
FUZZY_THRESHOLD = 80
...
def build_query(applicant_names: list[str], size: int = 50) -> dict:
    """
    Build a Lens API query for Y02 climate patents by applicant, 2010–2024.
        ...
        "query": {
            "bool": {
                "must": [
                    ...
                    # CPC class Y02* — wildcard to match all Y02 sub-codes
                    {
                        "query_string": {
                            "query": "class_cpc.symbol:Y02*"
                        }
                    },
...
def main():
    ...
    for parent, subs in firms.items():
        ...
        # Y02 sub-class distribution
        subclass_counts: dict[str, int] = {}
        for p in patents:
            for code in extract_y02_codes(p):
                prefix = code[:4]
        ...
        for p in patents:
            summary_rows.append({
                ...
                "y02_codes": "; ".join(extract_y02_codes(p)),
                "abstract": get_abstract(p)[:500],
            })

        if subclass_counts:
            print(f"    Y02 breakdown: {dict(sorted(subclass_counts.items()))}")
        print()
